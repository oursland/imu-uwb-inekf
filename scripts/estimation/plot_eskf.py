...
def plot_pos_eskf(t,Xpo,t_vicon,pos_vicon):
    ...
    plt.title(r"ESKF Position", fontsize=FONTSIZE,  color='black')
    plt.xlim(0, max(t))
...
def plot_vel_eskf(t,Xpo, t_vicon, vel_vicon):
    fig = plt.figure(facecolor="white",figsize=(10, 8))
    ax = fig.add_subplot(311)
    ...
    plt.legend(['Vicon ground truth','Estimate'])
    plt.title(r"ESKF Velocity", fontsize=FONTSIZE,  color='black')
    plt.xlim(0, max(t))
...
def plot_ori_eskf(t, q_list, t_vicon, ori_vicon):
    r_list = []
    for q in q_list:
    ...
    ax.plot(t, r_list[:,0], color='royalblue',linewidth=2.5, alpha=1.0)
    ax.set_ylabel(r'X [rad]',fontsize=FONTSIZE)
    plt.legend(['Vicon ground truth','Estimate'])
    plt.title(r"ESKF Orientation", fontsize=FONTSIZE,  color='black')
    plt.xlim(0, max(t))

    ax = fig.add_subplot(312)
    ax.plot(t_vicon, ori_vicon[:,1], color='orangered',linewidth=2.5,alpha=0.8)
    ax.plot(t, r_list[:,1], color='royalblue',linewidth=2.5, alpha=1.0)
    ax.set_ylabel(r'Y [rad]',fontsize=FONTSIZE)
    plt.xlim(0, max(t))

    ax = fig.add_subplot(313)
    ax.plot(t_vicon, ori_vicon[:,2], color='orangered',linewidth=2.5,alpha=0.8)
    ax.plot(t, r_list[:,2], color='royalblue',linewidth=2.5, alpha=1.0)
    ax.set_xlabel(r'time [s]',fontsize=FONTSIZE)
    ax.set_ylabel(r'Z [rad]',fontsize=FONTSIZE)
    plt.xlim(0, max(t))
    plt.ylim(-np.pi,np.pi)

def plot_pos_err_eskf(t,pos_error, Ppo=np.zeros((0, 9, 9))):
    ...
    fig = plt.figure(facecolor="white",figsize=(10, 8))
    ax = fig.add_subplot(311)
    plt.title(r"ESKF Position Error" , fontsize=FONTSIZE, fontweight=0, color='black')
...
def plot_vel_err_eskf(t, vel_error, Ppo=np.zeros((0, 9, 9))):
    # extract the variance
    D = Ppo.shape[0]
    delta_x = np.zeros([D,1])
    delta_y = np.zeros([D,1])
    ...
    fig = plt.figure(facecolor="white",figsize=(10, 8))
    ax = fig.add_subplot(311)
    plt.title(r"ESKF Velocity Error" , fontsize=FONTSIZE, fontweight=0, color='black')
...
def plot_ori_err_eskf(t, ori_error, Ppo=np.zeros((0, 9, 9))):
    # extract the variance
    D = Ppo.shape[0]
    delta_x = np.zeros([D,1])
    delta_y = np.zeros([D,1])
    ...
    fig = plt.figure(facecolor="white",figsize=(10, 8))
    ax = fig.add_subplot(311)
    plt.title(r"ESKF Orientation Error" , fontsize=FONTSIZE, fontweight=0, color='black')
...
def plot_traj_eskf(pos_vicon, Xpo, anchor_pos):
    ...
    ax_t.plot(Xpo[:,0], Xpo[:,1], Xpo[:,2],color='g', linewidth=1.0, alpha=1, label = 'ESKF estimation')
